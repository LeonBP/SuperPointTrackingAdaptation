...
class LightGlue(nn.Module):
    ...
    def _forward(self, data: dict) -> dict:
        ...
        kpts0, kpts1 = data["keypoints0"], data["keypoints1"]#data0["keypoints"], data1["keypoints"]
        b, m, _ = kpts0.shape
        b, n, _ = kpts1.shape
        device = kpts0.device
        size0, size1 = torch.tensor([1080.,1080.]),torch.tensor([1080.,1080.])#data0.get("image_size"), data1.get("image_size")
        kpts0 = normalize_keypoints(kpts0, size0).clone()
        kpts1 = normalize_keypoints(kpts1, size1).clone()

        desc0 = data["descriptors0"].transpose(-1,-2).detach().contiguous()#data0["descriptors"].detach().contiguous()
        desc1 = data["descriptors1"].transpose(-1,-2).detach().contiguous()#data1["descriptors"].detach().contiguous()
        ...
        pred = {
            ...
            "stop": torch.tensor([i + 1]).to('cuda:0'), # i+1
            "matches": matches,
            "scores": mscores,
            "prune0": prune0,
            "prune1": prune1,
