...
@torch.no_grad()
def export_detector_trackingAdapt_gpu(config, output_dir, args):
    """
    input 1 images, output pseudo ground truth by homography adaptation.
    Save labels:
        pred:
    ...
    data = dataLoader(config, dataset=task, export_task=export_task)
    test_set, test_loader = data["test_set"], data["test_loader"]
    recons = "/media/discoGordo/dataset_leon/colmap_2023/reconstruction_2023"
    points = {}
    visibles = {}
    recons_sg = "/media/discoGordo/dataset_leon/colmap_2023/reconstruction_2023_sg"
    points_sg = {}
    visibles_sg = {}
    names_sg = {}
    ...
    for seq in seqs:
        ...
        for subseq in subseqs:
            ...
            points[seq][subseq] = rec['points2d']
            visibles[seq][subseq] = rec['visible']
            if os.path.exists(os.path.join(recons_sg,seq,subseq,"points_projected.npz")):
                rec_sg = np.load(os.path.join(recons_sg,seq,subseq,"points_projected.npz"))
                points_sg[seq][subseq] = rec_sg['points2d']
                visibles_sg[seq][subseq] = rec_sg['visible']
                names_sg[seq][subseq] = rec_sg['names']
        '''points2d = points[seq][subseq]
        visible = visibles[seq][subseq]
        ...
        for j in range(visible.shape[1]):
            for i in range(visible.shape[0]):
                ...
                # z>0, inside the 256x256, visible or between others that are visible
                if points2d[i,j][2] > 0 and \
                        0 <= x < 256 and 0 <= y < 256 and \
                        (visible[i,j] or (True in visible[:i,j] and True in visible[i+1:,j])):
                    if pts is None:
                        pts = np.array([x,y,1])
                        pts = pts[np.newaxis]
                        pts_indexes = np.array([j])
                    else:
                        pts = np.vstack((pts,np.array([x,y,1])))
                        pts_indexes = np.hstack((pts_indexes,np.array([j])))'''
    ...
    for i, sample in tqdm(enumerate(test_loader)):
        ...
        points2d = points[seq][subseq]
        visible = visibles[seq][subseq]
        index = sample['image_index']
        index_sg = -1
        if subseq in names_sg[seq].keys():
            name_list = names_sg[seq][subseq]
            if name in name_list:
                index_sg = np.where(name_list == name)[0][0]
                points2d_sg = points_sg[seq][subseq]
                visible_sg = visibles_sg[seq][subseq]
        '''pts = None
        pts_indexes = None
        for j in range(visible.shape[1]):
            x = (points2d[index, j, 0] - x_offset) * scale
            y = (points2d[index, j, 1] - y_offset) * scale
            # if z>0 and inside the image space (256x256) and (visible or between others that are visible)
            if points2d[index,j][2] > 0 and \
                    0 <= x < img.shape[1] and 0 <= y < img.shape[0] and \
                ...
                # if visible[index,j]:
                #     break
        # print(pts, pts_indexes)
        # exit(1)
        for j in range(visible_sg.shape[1]):
            x = (points2d_sg[index_sg, j, 0] - x_offset) * scale
            y = (points2d_sg[index_sg, j, 1] - y_offset) * scale
            # if z>0 and inside the image space (256x256) and (visible or between others that are visible)
            if points2d_sg[index_sg,j][2] > 0 and \
                    0 <= x < img.shape[1] and 0 <= y < img.shape[0] and \
                    (visible_sg[index_sg,j] or (True in visible_sg[:index_sg,j] and True in visible_sg[index_sg+1:,j])):
                if pts is None:
                    pts = np.array([x,y,1])
                    pts = pts[np.newaxis]
                    pts_indexes = np.array([j])
        ...
        if pts is not None:
            ## save keypoints
            pred = {}
            pred.update({"pts": pts, 'pts_indexes': pts_indexes})'''
        ...
        for j in range(visible.shape[1]):
            ...
            # if z>0 and inside the image space (256x256) and (visible or between others that are visible)
            if points2d[index, j, 2] > 0 and \
                    0 <= x < img.shape[3] and 0 <= y < img.shape[2] and \
                    (visible[index, j] or (True in visible[:index, j] and True in visible[index + 1:, j])):
                pts.append([x, y, 1])
                pts_indexes.append(j)
        if index_sg >= 0:
            count_sg += 1
            for j in range(visible_sg.shape[1]):
                x = (points2d_sg[index_sg, j, 0] - x_offset) * scale
                ...
                if points2d_sg[index_sg, j, 2] > 0 and \
                        0 <= x < img.shape[3] and 0 <= y < img.shape[2] and \
                        (visible_sg[index_sg, j] or (
                                True in visible_sg[:index_sg, j] and True in visible_sg[index_sg + 1:, j])):
                    pts.append([x, y, 1])
                    pts_indexes.append(j+100000)

        pts = np.array(pts)
        pts_indexes = np.array(pts_indexes)
    ...
    print("output pseudo ground truth: ", count)
    print("output augmented with SuperGlue: ", count_sg)
    save_file = save_output / "export.txt"
    with open(save_file, "a") as myfile:
        myfile.write("Homography adaptation: " + str(homoAdapt_iter) + "\n")
        myfile.write("output pairs: " + str(count) + "\n")
...
if __name__ == "__main__":
    ...
    p_train.set_defaults(func=export_detector_trackingAdapt_gpu)
    ...
    output_dir = os.path.join('/media/discoGordo/dataset_leon/training_SP/', args.exper_name)#EXPER_PATH
