...
class Train_model_heatmap_tracking(Train_model_frontend):
    ...
    ** note: the input, output is different from that of SuperPointFrontend
    ...
    def train_val_sample(self, sample, n_iter=0, train=False):
        ...
        # variables
        batch_size, H, W = img1.shape[0], img1.shape[2], img1.shape[3]
        ...
        labels1_3D = labels2Dto3D(
            labels1_2D.to(self.device), cell_size=self.cell_size, add_dustbin=add_dustbin
        ).float()
        mask_3D_flattened = self.getMasks(mask1_2D, self.cell_size, device=self.device)
        loss_det1 = self.detector_loss(
            input=outs1["semi"],
            target=labels1_3D.to(self.device),
        ...
        if n_iter % tb_interval == 0 or task == "val":
            ...
            update_overlap(
                self.images_dict,
                labels1_2D,
                heatmap_org1_nms_batch[np.newaxis, ...],
                img1,
                "original1",
            )
            ...
            # )
            pr_mean = self.batch_precision_recall(
                to_floatTensor(heatmap_org1_nms_batch[:, np.newaxis, ...]),
                sample["labels1_2D"],
            )
            print("pr_mean")
