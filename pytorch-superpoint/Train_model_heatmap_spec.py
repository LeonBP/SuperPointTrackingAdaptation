...
class Train_model_heatmap_spec(Train_model_frontend):
    ...
    ** note: the input, output is different from that of SuperPointFrontend
    ...
    def specularities_loss(self, input, image):
        ...
        loss = (nn.functional.softmax(input, dim=1)[:,:-1,:,:] * torch.where(image>0.7,1,0)).sum(dim=[1,2,3])
        n_pixels=torch.where(image>0.7,1,0).sum()
