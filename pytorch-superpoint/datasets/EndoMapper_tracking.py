...
class EndoMapper_tracking(data.Dataset):
                ...
                for i in range(len(names)):
                    ...
                    if self.labels:
                        ...
                        if not p1.exists():
                            ...
                            points1 = np.load(p1, allow_pickle=True)
                            ind1 = points1['pts_indexes']
                            # pts1 = points1['pts']
                    for j in range(i+1,len(names)):
                        ...
                        if self.labels:
                            ...
                            if not p2.exists():
                                ...
                                tic = 0
                                tac = 0
                                while tic < len(ind1) and tac < len(ind2):
                                    if ind1[tic] == ind2[tac]:
                                        theyShare = True
                                        break
                                        # match = match + [(tic, tac)]
                                        # tic += 1
                                    elif ind1[tic] < ind2[tac]:
                                        tic += 1
                                    else:
                                        tac += 1
                                if not theyShare:  # len(match) == 0:  #
                                    continue
                                sample = {'image1': image_path1, 'name1': name1, 'points1': str(p1),
                                          'image2': image_path2, 'name2': name2, 'points2': str(p2)}
                                # 'match_indexes': match
    ...
    def __getitem__(self, index):
        ...
        def point_correspondences(pts1, pts2, pts1_indexes, pts2_indexes):
            ...
            while tic < len(pts1_indexes) and tac < len(pts2_indexes):
                if pts1_indexes[tic] == pts2_indexes[tac]:
                    if corres is None:
                        corres = np.hstack((pts1[tic, :2], pts2[tac, :2]))
                        corres = corres[np.newaxis]
                    else:
                        corres = np.vstack((corres, np.hstack((pts1[tic, :2], pts2[tac, :2]))))
                    tic += 1
        ...
        # laebls
        if self.labels:
            points1 = np.load(sample['points1'])
            points2 = np.load(sample['points2'])
            pnts1 = points1['pts']
            pnts2 = points2['pts']
            # pnts = pnts.astype(int)
            ...
            pnts1_indexes = points1['pts_indexes']
            pnts2_indexes = points2['pts_indexes']
            matches = point_correspondences(pnts1, pnts2, pnts1_indexes, pnts2_indexes)
            # print(sample['name1'])
            # print(sample['name2'])
            matches = matches[np.newaxis]
            # print(correspondences.shape)
            input.update({'matches': matches})
