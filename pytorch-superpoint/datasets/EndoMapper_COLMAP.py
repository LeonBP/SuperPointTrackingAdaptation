...
class EndoMapper_COLMAP(data.Dataset):
    default_config = {
        'labels': None,
                ...
                with open(os.path.join(recons, seq, subseq, "images.txt"), "r") as myfile:
                    ...
                    for i in range(4, len(lines)):
                        if i % 2 == 0:
                            ...
                            names = names + [name]
                            image_paths = image_paths + [os.path.join(base_path, seq, subseq, name)]
    ...
    def __getitem__(self, index):
        ...
        input.update(sample)
        seq = sample['name'][:7]
        subseq = str(int(sample['name'][8:11]))
        image_list = self.image_lists[seq][subseq]
        for i in range(len(image_list)):
            if sample['name'] == image_list[i]:
                input.update({'image_index': i})
                break
        #print(sample['name'], input['image_index'])
        #exit(1)
        # image
