...
class EndoMapper_tracking(data.Dataset):
    default_config = {
        'labels': None,
    ...
    def __init__(self, export=False, transform=None, task='train', **config):
        ...
        for seq in seqs:
            ...
            for subseq in subseqs:
                ...
                names = []
                image_paths = []
                with open(os.path.join(recons,seq,subseq,"images.txt"), "r") as myfile:
                    lines = myfile.readlines()
                    for i in range(4,len(lines)):
                        if i % 2 == 0:
                            line = lines[i].split()
                            name = line[-1][:8]+subseq.zfill(3)+line[-1][-8:]
                            names = names + [name]
                            full_path = os.path.join(base_path, seq, subseq, name)
                            if os.path.exists(full_path):
                                image_paths = image_paths + [full_path]
                                images[seq][subseq] = images[seq][subseq] + [name]
                for i in range(len(names)):
                    for j in range(i+1,len(names)):
                        names1 = names1 + [names[i]]
                        image_paths1 = image_paths1 + [image_paths[i]]
                        names2 = names2 + [names[j]]
                        image_paths2 = image_paths2 + [image_paths[j]]
    ...
    def __getitem__(self, index):
        ...
        def point_correspondences(pts1, pts2, pts1_indexes, pts2_indexes):
            corres = None
            for i in range(len(pts1_indexes)):
                for j in range(len(pts2_indexes)):
                    if pts1_indexes[i] == pts2_indexes[j]:
                        if corres is None:
                            corres = np.hstack((pts1[i, :2],pts2[j, :2]))
                            corres = corres[np.newaxis]
                        else:
                            corres = np.vstack((corres, np.hstack((pts1[i, :2],pts2[j, :2]))))
            return corres
        ...
        img1_o = _read_image(sample['image1'])
        img2_o = _read_image(sample['image2'])
        H, W = img1_o.shape[0], img1_o.shape[1]
        # print(f"image: {image.shape}")
        img1_aug = img1_o.copy()
        img2_aug = img2_o.copy()
        if (self.enable_photo_train == True and self.action == 'train') or (self.enable_photo_val and self.action == 'val'):
            img1_aug = imgPhotometric(img1_o) # numpy array (H, W, 1)
            img2_aug = imgPhotometric(img2_o) # numpy array (H, W, 1)


        # img_aug = _preprocess(img_aug[:,:,np.newaxis])
        img1_aug = torch.tensor(img1_aug, dtype=torch.float32).view(-1, H, W)
        img2_aug = torch.tensor(img2_aug, dtype=torch.float32).view(-1, H, W)

        valid_mask = self.compute_valid_mask(torch.tensor([H, W]), inv_homography=torch.eye(3))
        input.update({'image1': img1_aug})
        input.update({'image2': img2_aug})
        ...
        # laebls
        if self.labels:
            pnts1 = np.load(sample['points1'])['pts']
            pnts2 = np.load(sample['points2'])['pts']
            # pnts = pnts.astype(int)
            # labels = np.zeros_like(img_o)
            # labels[pnts[:, 1], pnts[:, 0]] = 1
            labels1 = points_to_2D(pnts1, H, W)
            labels1_2D = to_floatTensor(labels1[np.newaxis,:,:])
            input.update({'labels1_2D': labels1_2D})
            labels2 = points_to_2D(pnts2, H, W)
            labels2_2D = to_floatTensor(labels2[np.newaxis,:,:])
            input.update({'labels2_2D': labels2_2D})
            pnts1_indexes = np.load(sample['points1'])['pts_indexes']
            pnts2_indexes = np.load(sample['points2'])['pts_indexes']
            correspondences = point_correspondences(pnts1, pnts2, pnts1_indexes, pnts2_indexes)
            # print(sample['name1'])
            # print(sample['name2'])
            correspondences = correspondences[np.newaxis]
            # print(correspondences.shape)
            input.update({'correspondences': correspondences})
            ...
            if self.gaussian_label:
                ...
                labels1_gaussian = self.gaussian_blur(squeezeToNumpy(labels1_2D))
                labels1_gaussian = np_to_tensor(labels1_gaussian, H, W)
                input['labels1_2D_gaussian'] = labels1_gaussian
                labels2_gaussian = self.gaussian_blur(squeezeToNumpy(labels2_2D))
                labels2_gaussian = np_to_tensor(labels2_gaussian, H, W)
                input['labels2_2D_gaussian'] = labels2_gaussian
        ...
        input.update({'name1': name1, 'name2': name2, 'scene_name': "./"}) # dummy scene name
        return input
