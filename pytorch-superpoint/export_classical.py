...
def get_sift_match(sift_kps_ii, sift_des_ii, sift_kps_jj, sift_des_jj, if_BF_matcher=True):
    ...
    if len(quality_good.shape)==1:
        quality_good=good_ij.copy()
    if len(quality_all.shape)==1:
        quality_all=good_ij.copy()
...
def get_sift_match_idx_pair(sift_matcher, des1, des2):
    ...
    elif len(matches)>0 and len(matches[0])>0:
        for m in matches:
            all_m.append(m[0])
            good.append(m[0])
            quality_good.append([m[0].distance, m[0].distance])
            quality_all.append([m[0].distance, m[0].distance])
