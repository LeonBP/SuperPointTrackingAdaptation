...
def descriptor_loss_sparse(descriptors, descriptors_warped, homographies, mask_valid=None,
    ...
    # print(non_matches_a.min(), non_matches_a.max(), non_matches_b.min(), non_matches_b.max(), image_a_pred.size(),image_b_pred.size(),non_matches_a.size(),non_matches_b.size())
    # exit(1)
    ...
    # print(match_loss,non_match_loss,lamda_d)
    # exit(1)
...
def descriptor_loss_tracking(descriptors1, descriptors2, correspondences, mask_valid=None,
    ...
    Hc, Wc = descriptors1.shape[1], descriptors1.shape[2]
    img_shape = (Hc, Wc)
    # print("img_shape: ", img_shape)
    # img_shape_cpu = (Hc.to('cpu'), Wc.to('cpu'))
    ...
    image_a_pred = descriptor_reshape(descriptors1)  # torch [1, H*W, D]
    # print("image_a_pred: ", image_a_pred.shape)
    image_b_pred = descriptor_reshape(descriptors2)  # torch [batch_size, H*W, D]
    correspondences = correspondences.squeeze(0)
    # matches
    ...
    if method == '2d':
        match_loss = get_match_loss(descriptors1, descriptors2, matches_a.to(device),
            matches_b.to(device), dist=dist, method='2d')
    else:
        match_loss = get_match_loss(image_a_pred, image_b_pred,
            matches_a.long().to(device), matches_b.long().to(device), dist=dist)
...
def batch_descriptor_loss_tracking(descriptors1, descriptors2, correspondences, **options):
    loss = []
    pos_loss = []
    neg_loss = []
    batch_size = descriptors1.shape[0]
    for i in range(batch_size):
        losses = descriptor_loss_tracking(descriptors1[i], descriptors2[i],
                    # torch.tensor(homographies[i], dtype=torch.float32), **options)
                    correspondences[i].type(torch.float32), **options)
        loss.append(losses[0])
        pos_loss.append(losses[1])
        neg_loss.append(losses[2])
    loss, pos_loss, neg_loss = torch.stack(loss), torch.stack(pos_loss), torch.stack(neg_loss)
