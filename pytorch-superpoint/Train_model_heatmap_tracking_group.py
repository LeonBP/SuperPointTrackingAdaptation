...
class Train_model_heatmap_tracking_group(Train_model_frontend):
    ...
    def train_val_sample(self, sample, n_iter=0, train=False):
        ...
        for i in range(len(labels_2D)):
            labels_3D_list.append(labels2Dto3D(
                labels_2D[i].to(self.device), cell_size=self.cell_size, add_dustbin=add_dustbin
            ).float())
            ...
            if loss is None:
                ...
                loss += loss_det_list[i]
            # labels2_3D = labels2Dto3D(
            #     labels2_2D.to(self.device), cell_size=self.cell_size, add_dustbin=add_dustbin
            # ).float()
            # # mask2_3D_flattened = self.getMasks(mask2_2D, self.cell_size, device=self.device)
            # loss_det2 = self.detector_loss(
            #     input=outs2["semi"],
            #     target=labels2_3D.to(self.device),
            #     mask=mask_3D_flattened,
            #     loss_type=det_loss_type,
            # )
            '''loss_spec = self.specularities_loss(
                input=outs["semi"],
            ...
            '''if if_warp:
                ...
                loss_spec_warp = self.specularities_loss(
                    input=outs_warp["semi"],
                    image=img_warp.to(self.device)
                )
            else:
                loss_det_warp = torch.tensor([0]).float().to(self.device)
                loss_spec_warp = torch.tensor([0]).float().to(self.device)'''
